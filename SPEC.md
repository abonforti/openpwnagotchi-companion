...
| # | Decision | Value |
|---|---|---|
| D1 | Backend | Fork of `BraedenP232/PwnIOS` `pwnios.py`, hardened. GPL-3.0, attribution preserved. Upstream declares GPL3 in the file and MIT in its LICENSE (F34); this project is GPL-3.0 under either reading and retains the MIT notice verbatim in `NOTICE` (`docs/LICENSING.md`, issue #25). |
...
│   ├── LICENSING.md                  # upstream declares two licences; what that means here
...
├── CONTRIBUTING.md
├── LICENSE                           # GPL-3.0
├── NOTICE                            # upstream's MIT notice, retained verbatim (D1, F34)
...
**A client is dropped when its send fails, whichever way it fails (issue #243).** `broadcast()`
hands each `client.send(payload)` to the loop with `run_coroutine_threadsafe`. That call can
fail in two places, and both drop the client from the `ClientSet`: synchronously, when the
coroutine object cannot even be built, which is the shape a client with a broken `send`
produces; and on the loop, when the awaited coroutine raises or is cancelled, which is what a
closed WebSocket does. The second is seen through the future the scheduling call returns: a
done-callback discards the client when the future holds an exception or was cancelled. The hook
thread never waits on that future, so a slow client still costs the caller nothing, and a
successful send leaves the set alone. The method's docstring names both failures. Before this,
every phone that walked away stayed in the set, and every later broadcast scheduled a send to a
socket nobody read.
...
- **A symlink inside the web root is followed only if it lands inside the web root (issue
  #244).** `translate_path` compares real paths, not request paths: the web root is resolved
  with `os.path.realpath` once, when the handler class is built, and the answer for a request is
  resolved the same way before the comparison. The comparison is on path components, through
  `os.path.commonpath`, never on a string prefix, so a sibling directory whose name merely
  starts with the root's is outside it. A path that resolves outside the root is answered with
  `index.html`, the same fallback a traversal that slipped past the base class would get, and the
  file the link pointed at is never opened. That last sentence rests on the root being written
  by root alone, as `install-on-pi.sh` leaves it: the check reads the real path and the base
  class then opens the request path, so a link swapped between the two would be a race only
  someone who can already write the root could run. A dangling link is resolved the same way,
  since the resolution does not require the target to exist: one whose target would sit outside
  the root gets `index.html`, and one whose target would sit inside it reaches the base class
  and is a missing file there. Resolving rather than refusing every link is the decision: the
  root itself may be a symlink an owner set up, and a link that stays inside the root discloses
  nothing the root did not already hold. The guard's comment says that it compares real paths
  by component, not request paths by prefix. The fallback gets the same check as any other
  answer (issue #256): `translate_path` resolves `<root>/index.html` too, and if the shell
  itself is a link whose target lies outside the root there is nothing in the root left to
  serve, so the guard answers with a path no file can have and `send_head` sends the 404
  itself, before the base class looks at the path and whether the link points at a file or at
  a directory. That last distinction is why the 404 is not left to the base class: its own
  not-found for such a path sits behind a directory check, so a link at a directory would be
  redirected into it instead, and one spelling of the slash walks it and serves its index page
  on 3.13. Before that, the fallback was the one path handed back unresolved, and a shell
  linked out of the root would have been served for every request the guard redirected and
  every single-page route. The shell the guard answers with is never
  cacheable, whichever request produced it (issue #253): `send_head` notices that
  `translate_path` answered with the fallback and rewrites the request path to `/index.html`,
  so `end_headers` applies the same `Cache-Control: no-cache` a direct request for the shell
  gets. Before that, the shell served under a link out of the root carried no cache rule at
  all, and a copy cached under that name was left to the cache's own heuristics to refresh.
...
roadmap (§8), and GPL-3.0 with attribution to `BraedenP232/PwnIOS`, saying plainly that
upstream declares two licences and pointing at `docs/LICENSING.md` for what that means here.
...
| `test_static_server.py` | `.webmanifest` and `.js` content types; SPA fallback serves `index.html`; `..` rejected with 400; no directory listing; `no-cache` on index and service worker; `text/html; charset=utf-8` on a direct `index.html` request and on the SPA fallback alike (SPEC 4.2, SPEC 2.15); a symlink inside the web root pointing outside it falls back to `index.html` rather than serving the target, a symlink that stays inside the root is followed, a sibling directory whose name merely starts with the root's is treated as outside it, the web root itself may be a symlink, and a dangling link is resolved the same way, so one whose target would sit outside the root gets the shell while one whose target would sit inside it is a missing file (SPEC 2.15, issue #244); the shell served in place of a link out of the root, whether the link sits directly in the root or behind a sibling-prefix directory, carries `Cache-Control: no-cache` the same as a direct request for it would, and an ordinary hashed asset stays cacheable (SPEC 2.15, issue #253); a shell that is itself a link out of the root is refused with 404 for a direct request, a single-page route and a missing asset alike, a shell that is a link staying inside the root still serves the direct request and the fallback both, and a request the guard redirects to a shell linked out of the root is refused too rather than landing on the outside shell, and a shell replaced by a link out of the root while the server is running is refused from then on, since the fallback is resolved per request and not once at construction (SPEC 2.15, issue #256) |
