...
pwnagotchi never calls `on_ready`, so the agent never arrives. Not an omission upstream could
fix by remembering: the manual path does not run the code that fires the event. `ready` is
emitted from one place, reached through `agent.start()`, and `do_manual_mode` never calls it
(F31). Two consequences, and both were defects (issue #140).
...
| F31 | **`on_ready` is never fired in manual mode, because the manual path does not reach the code that fires it.** `plugins.on('ready', self)` is emitted from `automata.py`, reached through `agent.start()`. A grep over the package at 2.9.5.8 found it nowhere else, which is an observation rather than something the checker can hold: a per-file check cannot pin package-wide uniqueness. `cli.py` has two entry points: `do_manual_mode` sets `agent.mode = 'manual'`, parses the last session, and enters a `while True` that sleeps and emits `internet_available`; `do_auto_mode` sets `agent.mode = 'auto'` and calls `agent.start()`. So a plugin in manual mode receives no agent, ever, which is why this one starts its threads in `on_loaded` (§2.5, issue #140). Note that manual mode is not hookless: that loop does emit `internet_available` every five seconds. **Machine-checked entries**: F31a, F31b pin that `automata.py` is where `ready` is emitted and that `cli.py` never emits it; that the manual branch does not call `agent.start()` is read from the two function bodies and is not machine-checked. | `pwnagotchi/automata.py`, `plugins.on('ready', self)`; `pwnagotchi/cli.py`, `do_manual_mode` and `do_auto_mode`, observed on 2.9.5.8 |
